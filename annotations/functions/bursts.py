...
def return_envelope_diff(acc, envelope = True, plot = False, alfa = 15):
    """
    ...
    if envelope:
        ...
        if len(lmax) > len(lmin):
            lmax = lmax[1:]
        envelope_diff = pd.Series(acc.values[lmax] - acc.values[lmin], index = acc.index[lmax])

    return envelope_diff