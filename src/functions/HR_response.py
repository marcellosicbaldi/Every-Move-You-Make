...
def detect_HR_change_from_RR(bursts, hr_df, off, plot = False):
    ...
    -------
    HR_change : list
        List with the HR change for each burst
    latency_HR_change : list
        List with the latency of the HR change for each burst
    """
    HR_bursts_df = []
    HR_change = []
    posture_change = []
    ...
    for i in range(len(bursts)):
        ...
        HR_change.append(df_burst["HR"].iloc[20:].max())
        # latency_HR_change.append(df_burst["HR"].idxmax() - bursts["start"].iloc[i])
    ...
    return HR_bursts_df, np.array(HR_change), np.array(AUC), np.array(posture_change)
